SCRIPT_TITLE = "Create Parameter Anchors"
...
function getTranslations(langCode)
  if langCode == "ja-jp" then
    return {
      ...
      {"Place third point at center of note.", "音符の中心に 3 点目を置きます。"},    	  
  ...
  if langCode == "zh-cn" then
    return {
      ...
      {"Place third point at center of note.", "将第三点放在音符的中心。"},    	  
	...
	if #selectedNotes == 0 then
		...
		--if nil then
			...
			widget_build[#widget_build + 1] = {
				name = "midpoint",
				type = "CheckBox",
				text = SV:T("Place third point at center of note."),
				default = false,
				}
...
Pretty straight-forward. If you assign a voice to a track and sdave it, this will be abler to add anchors to the voice mode parameters as well.
...
function getTranslations(langCode)
  if langCode == "ja-jp" then
    return {
      {"anchorize Parameters", "パラメータの乱数化"},
      {"Parameter Type", "パラメータタイプ"}
    }
  end
  if langCode == "zh-cn" then
    return {
      {"anchorize Parameters", "参数随机化"},
    }
  end
  return {}
end
